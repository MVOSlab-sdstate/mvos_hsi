...
from .utils import load_wavelengths
...
def _load_cube_any(path: str, key: Optional[str]) -> Tuple[np.ndarray, Optional[Dict]]:
    ...
    if ext == ".mat":
        ...
        if chosen is None:
            raise KeyError(
                f"No suitable cube key found in {path}. "
...
def plot_leaf_center(
    ...
    *,
    ...
    belonging to a single stem.

    Looks for files like:
        <clipped_dir>/<stem>_leaf<N>.hdr
    """
...
def plot_leaf_centers_multi(
    ...
    *,
    ...
    """
    Plot spectra from the CENTER pixel of multiple (stem, leaf#) pairs
    on one figure.

    Looks for:
        <clipped_dir>/<stem>_leaf<leaf>.hdr
    """
    curves: List[np.ndarray] = []
    labels: List[str] = []
    wl_global = None
...
def main(argv: Optional[List[str]] = None):
    """
    CLI entry for plotting (legacy). For unified CLI, use `mvos-hsi plotting ...`.
    """
    import argparse

    p = argparse.ArgumentParser(
        prog="mvos-hsi-plot",
        description="Plot spectral profiles from hyperspectral images",
    )
    sub = p.add_subparsers(dest="cmd", required=True)

    # pixel
    s1 = sub.add_parser("pixel", help="Plot a spectrum at a pixel (x,y)")
    s1.add_argument("--image", required=True, help="Path to .mat or .hdr")
    s1.add_argument("--x", type=int, required=True)
    s1.add_argument("--y", type=int, required=True)
    s1.add_argument("--key", help="For .mat: data key")
    ...
    # roi
    s2 = sub.add_parser("roi", help="Plot spectrum averaged over rectangle (x0,y0)-(x1,y1)")
    ...
    # leaf-center (single stem)
    s3 = sub.add_parser("leaf", help="Center spectrum from clipped leaf(s) for one stem")
    s3.add_argument("--clipped-dir", required=True)
    s3.add_argument("--stem", required=True)
    s3.add_argument("--leaf", nargs="+", type=int, required=True)
    ...
    # leaf-multi (different stems)
    s4 = sub.add_parser("leaf-multi", help="Center spectra from multiple (stem, leaf) pairs")
    s4.add_argument("--clipped-dir", required=True)
    s4.add_argument("--item", action="append", required=True,
                    help="Pair as STEM:LEAF (repeatable). Example: --item H_P1_V4_B:1")
