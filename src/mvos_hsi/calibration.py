...
def _read_envi_cube(base: str, suffix: str) -> np.ndarray:
    ...
    for img in img_candidates:
        if os.path.exists(img):
            shape = (
                N_LINE_SAMPLE if suffix.upper() == "R" else N_LINE_DARK,
                N_SAMPLE,
                N_BAND,
            )
...
def _calibrate_pair(
    ...
    """
    Core calibration routine for one sample/dark pair.

    Parameters
    ----------
    sample_base : str
        Base path for sample (without _R/_F and extension).
    dark_base : str
        Base path for dark reference (without _R/_F and extension).
    spectral_bin : int
        Spectral binning factor (1=no binning).
    spatial_bin : int
        Spatial binning factor along sample dimension.

    Returns
    -------
    dict
        Keys: R_plant, F_plant, M_plant, RS_plant, FS_plant.
    """
    ...
    R_mask = M_plant & (~bad)
    F_mask = M_plant  # keep same as MATLAB-style
...
def calibrate(
    ...
    """
    Calibrate a single sample/dark pair and return calibrated arrays.
    """
...
def calibrate_folder(
    ...
    """
    Calibrate all samples in a folder (skipping the dark).

    Saves `<stem>_R.mat` and `<stem>_F.mat` for each sample and returns a list
    of calibration dicts (same keys as `calibrate`).
    """
    _check_factor(spectral_bin, "spectral_bin")
    _check_factor(spatial_bin,  "spatial_bin")
    ...
    candidates = set()
    for p in glob.glob(os.path.join(folder, "*_R.*")):
        candidates.add(p)
    for p in glob.glob(os.path.join(folder, "*_r.*")):
        candidates.add(p)
    ...
    for rfile in sorted(candidates):
        ...
        # save calibrated cubes for future clipping/plotting
...
def main(argv: Optional[List[str]] = None):
    """
    CLI entry for calibration (legacy). For unified CLI, see mvos_hsi.cli.
    """
    import argparse
    ...
    if args.cmd == "calibrate":
        ...
        if args.k == 0:
            raise ValueError("spectral bin 0 is not allowed.")
        outs = calibrate_folder(
            args.folder,
            args.dark,
            spectral_bin=args.k,
            spatial_bin=args.spatial,
